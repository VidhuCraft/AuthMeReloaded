...
public class CacheDataSource implements DataSource {
	...
	@Override
	public boolean updateisActivated(PlayerAuth auth) {
		if(source.updateisActivated(auth)){
			cache.get(auth.getNickname()).setIsActivated(auth.getIsActivated());
			return true;
		}
		return false;
	}
	...
	@Override
	public boolean updateActivationKey(PlayerAuth auth) {
		if(source.updateActivationKey(auth)){
			cache.get(auth.getNickname()).setActivationKey(auth.getActivationKey());
			return true;
		}
		return false;
	}
	...
	@Override
	public String getActivationkey(PlayerAuth auth) {
		throw new UnsupportedOperationException("Please diable cache for activation to work");
	}

	@Override
	public boolean doActivation(PlayerAuth auth) {
		throw new UnsupportedOperationException("Please diable cache for activation to work");
	}
